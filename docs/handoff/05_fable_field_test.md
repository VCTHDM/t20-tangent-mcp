...
19. ~~楼梯梯段 `TLStair` / `TAStair`~~ **已完成**（2026-06-14，Handoff 25：
    封装为 `line_stair` / `arc_stair`，命令行单点循环插入(点取位置→回车退出)无弹框无选对象，
    E2E 生成 `TCH_LINESTAIR` / `TCH_ARCSTAIR`；梯段宽/踏步数/半径走面板记忆值）。
20. 电梯 `TElevator` 已探测暂不封装（2026-06-14，Handoff 25）：电梯间角点→对角点→
    点取开电梯门的墙线(选墙线步，需前置墙，空回车则 0 实体)，前置重+选对象步，暂搁置。
    双跑楼梯 `TRStair` 等预计带 #32770 面板或选墙，未探先记。
