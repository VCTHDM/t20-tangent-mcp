...
> 当前项目由两个模型交替推进：谁接手就先读 README、相关 handoff 与当前 diff，再继续下一步。
> 没有上下级分工；但真机安全门禁仍保留：未 E2E 或事故相关命令只允许 dry-run，不开放 execute。
...
commit 前缀按实际执行者标记即可；同一批修改必须在 handoff 中写清楚现场、验证结果和下一步。

**规则 1 —— 安全边界：以下情况不得硬闯，必须先记录现场再交替接力：**
...
**规则 2 —— 可按管线推进，但触发即停：**
照「封装方法论」五步管线（见上文）可完成的活可以由任一模型继续：批量封装、属性探测、
