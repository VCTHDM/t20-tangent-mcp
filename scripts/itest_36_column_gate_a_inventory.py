"""真机联调 Step 36 — TGColumn / column #32770 标准柱面板 Gate A inventory.

目标 (Handoff 32 P2 路线 / Gate A inventory-only):
    捕获 TGColumn 弹出的 #32770 标准柱面板的窗口结构 (class / title / styles /
    enabled state / 子控件树), 为后续 P3 UI 选择路线提供事实证据。
...
复测目标:
    1. TGColumn 仍然弹 #32770 (沿用 Handoff 13 / itest_30 结论);
...
用法: uv run python scripts/itest_36_column_gate_a_inventory.py
"""
...
def is_modal_owner_disabled(modal_hwnd: int) -> tuple[int, bool]:
    """返回 (owner_hwnd, owner_is_disabled). 真模态会让 owner 处于 disabled."""
...
def enum_children(parent_hwnd: int, depth: int = 2) -> list[dict]:
    """递归枚举子控件 (limited depth) 的 class/title/style/rect/enabled。"""
    out: list[dict] = []
...
async def main() -> int:
    ...
    # --- 注册确认 ---
    cn = await backend.execute_lisp('(strcat "cn=" (vl-prin1-to-string (getcname "TGCOLUMN")))')
    ...
    # --- launch TGCOLUMN ---
    r = await backend.execute_lisp(START_TGCOLUMN)
    ...
    # --- ESC 恢复 ---
    print()
    print("[recover] ESC-only sequence")
    for h in modals:
        for _ in range(4):
            post_escape(h, 4)
            time.sleep(0.2)
    ...
    print()
    print("=" * 60)
    print("TGColumn Gate A inventory")
    print("=" * 60)
    ...
    no_entity_created = after_cnt == base_cnt
    print()
    print("=== Step36 Gate A verdict ===")
    print(f"  TGColumn 弹 #32770 面板 (复现 Handoff 13): {'YES' if has_modal else 'no'}")
    print(f"  ESC 后无残留模态                           : {'YES' if no_residual else 'NO'}")
    print(f"  环境恢复 (CMDACTIVE=0)                     : {'YES' if env_clean else 'no'}")
    print(f"  无实体生成 (entity delta=0)                : {'YES' if no_entity_created else 'no'}")
    print(f"  -> 结论: {'BLOCKED-by-#32770 (Gate A inventory captured)' if (has_modal and no_residual and env_clean and no_entity_created) else 'INDETERMINATE'}")
    return 0 if (has_modal and no_residual and env_clean and no_entity_created) else 2
