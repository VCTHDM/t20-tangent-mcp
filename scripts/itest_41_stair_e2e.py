"""真机联调 Step 41 — line_stair / arc_stair 楼梯梯段验收.

均为命令行单点循环插入, 无对话框, 无选对象步:
- line_stair  TLSTAIR  点取位置->回车退出循环  -> TCH_LINESTAIR
- arc_stair   TASTAIR  点取位置->回车退出循环  -> TCH_ARCSTAIR

梯段宽/踏步数/半径等构造参数走天正面板记忆值, 只参数化插入点。
E2E 校验「实体增加 且 entlast 类型符合」。

用法: uv run python scripts/itest_41_stair_e2e.py
"""
...
CASES = [
    ("line_stair", {"x": 0, "y": 0},       "TCH_LINESTAIR"),
    ("arc_stair",  {"x": 8000, "y": 0},    "TCH_ARCSTAIR"),
]
