"""真机联调 Step 43 — double_stair / multi_stair 楼梯整体验收.

均为命令行点序列, 无对话框, 无选对象步 (2026-06-14 itest_42 侦察确认):
- double_stair  TRSTAIR      插入点->回车退出循环          -> TCH_RECTSTAIR
- multi_stair   TMULTISTAIR  起点->下一点->回车退出        -> TCH_MULTISTAIR

跑数/梯段宽/踏步数/楼梯高等构造参数走天正面板记忆值, 只参数化插入点/方向点。
E2E 校验「实体增加 且 entlast 类型符合」。

用法: uv run python scripts/itest_43_double_multi_stair_e2e.py
"""
...
CASES = [
    ("double_stair", {"x": 0, "y": 0},                              "TCH_RECTSTAIR"),
    ("multi_stair",  {"x1": 12000, "y1": 0, "x2": 12000, "y2": 6000}, "TCH_MULTISTAIR"),
]
