...
async def main() -> int:
    ...
    print("\n【门】")
    await execute_tangent(
        backend, "door", {"ins_x": 4500, "ins_y": 0, "width": 1000, "height": 2100}
    )
    await execute_tangent(
        backend, "door", {"ins_x": 9600, "ins_y": 4500, "width": 900, "height": 2100}
    )
    await execute_tangent(
        backend, "door", {"ins_x": 6000, "ins_y": 4500, "width": 900, "height": 2100}
    )
    await execute_tangent(
        backend, "door", {"ins_x": 9000, "ins_y": 7500, "width": 900, "height": 2100}
    )
    await execute_tangent(
        backend, "door", {"ins_x": 3600, "ins_y": 7000, "width": 800, "height": 2100}
    )
    ...
    await execute_tangent(
        backend,
        "window",
        {"ins_x": 6600, "ins_y": 0, "width": 2400, "height": 1800, "sill_height": 300},
    )
    await execute_tangent(
        backend,
        "window",
    ...
    for t, n in sorted(by_type.items()):
        print(f"  {t:30s} {n}")
    print("\n=== 按图层统计 ===")
    for layer_name, n in sorted(by_layer.items()):
        print(f"  {layer_name:30s} {n}")
